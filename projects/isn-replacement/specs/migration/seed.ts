...
async function main() {
  ...
  // ---- Write .env.migration for downstream scripts ----
  // Saves MIGRATION_ACCOUNT_ID and MIGRATION_SAFEHOUSE_BIZ_ID so subsequent
  // scripts can source it instead of copying values from stdout.
  // Loaded via `source migration/.env.migration` or `dotenv.config({ path: 'migration/.env.migration' })`.
  const envContent = [
    `# Auto-generated by seed.ts on ${new Date().toISOString()}`,
    `# Source this file before running migration scripts:`,
    `#   set -a && source migration/.env.migration && set +a`,
    `MIGRATION_ACCOUNT_ID=${accountId}`,
    `MIGRATION_SAFEHOUSE_BIZ_ID=${businessIds["safehouse"] ?? ""}`,
    `MIGRATION_HCJ_BIZ_ID=${businessIds["hcj-pools"] ?? ""}`,
    `MIGRATION_PEST_BIZ_ID=${businessIds["pest-heroes"] ?? ""}`,
    `SEED_ACCOUNT_SLUG=${ACCOUNT_SLUG}`,
    "",
  ].join("\n");

  const envPath = "migration/.env.migration";
  const fs = await import("fs");
  const envDir = "migration";
  if (!fs.existsSync(envDir)) fs.mkdirSync(envDir, { recursive: true });
  fs.writeFileSync(envPath, envContent, "utf8");
  log("seed", `Wrote ${envPath} — source this before running migration scripts`);
